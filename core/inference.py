...
def extract_attrs(proc, model, img):
    ...
    s = proc.batch_decode(out, skip_special_tokens=True)[0]
    try:
...
def pair_rerank(proc, model, img_a, img_b):
    ...
    msgs = [
      {"role":"user","content":[{"type":"image","image":img_a},{"type":"text","text":"첫 번째 이미지"}]},
      {"role":"user","content":[{"type":"image","image":img_b},{"type":"text","text":"두 번째 이미지"}]},
      {"role":"user","content":[{"type":"text","text":prompt}]}
    ]
    text = proc.apply_chat_template(msgs, add_generation_prompt=True)
    inputs = proc(text=[text], images=[img_a, img_b], return_tensors="pt", padding=True).to(model.device)
